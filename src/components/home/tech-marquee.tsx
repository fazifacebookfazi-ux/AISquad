...
export function TechMarquee() {
  return (
    <section className="border-y border-white/[0.07] bg-ink-900/60 py-10">
      <Container>
      ...
      <div className="relative mt-7 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_12%,black_88%,transparent)]">
        <div className="animate-marquee flex w-max gap-4 pr-4">
          {[...stack, ...stack].map((item, i) => (
            <span
              key={`${item}-${i}`}
              className="rounded-full border border-white/[0.08] bg-white/[0.03] px-5 py-2.5 text-sm whitespace-nowrap text-mist-300"
            >
              {item}
            </span>
          ))}
